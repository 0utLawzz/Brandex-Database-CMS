...
function FilterField({ label, children }: { label: string; children: React.ReactNode }) {
  return <label className="flex flex-col gap-1.5"><span className="font-mono text-sm font-bold uppercase tracking-widest text-[#6d6658]">{label}</span>{children}</label>;
}
const filterControl = "h-9 px-2 bg-white border border-stone-300 font-mono text-sm focus:outline-2 focus:outline-[#6C1C1F] min-w-[105px]";
  ...
  return (
    <AppShell>
      <div className="flex flex-col h-full bg-white">
        <div className="shrink-0 flex flex-wrap items-center gap-3 px-4 sm:px-6 py-3 bg-[#E8DFC7] border-b border-stone-300">
          ...
          <span className="font-mono text-sm text-[#6d6658] font-bold uppercase tracking-widest hidden md:inline">{isLoading || isFetching ? "LOADING…" : `${total} RECORDS`}</span>
          <div className="relative order-last sm:order-none w-full sm:w-64">
            ...
            <input value={filters.search} onChange={(event) => setFilter("search", event.target.value)} placeholder="Search any key field…" className="w-full h-10 pl-9 pr-9 bg-white border border-stone-300 font-mono text-sm focus:outline-2 focus:outline-[#6C1C1F]" />
          ...
          <button onClick={() => setShowFilters((value) => !value)} className={`flex items-center gap-2 px-3 h-10 border border-stone-300 font-mono font-bold text-sm uppercase tracking-wider ${showFilters || hasFilters ? "bg-[#0C0C0C] text-[#F0E8D0]" : "bg-white"}`}><Filter className="w-4 h-4" /> FILTERS{hasFilters ? " ●" : ""}</button>
          <button onClick={handleExport} disabled={exporting || total === 0} className="flex items-center gap-2 px-3 h-10 bg-white border-2 border-[#6C1C1F] text-[#6C1C1F] font-mono font-bold text-sm uppercase tracking-wider disabled:opacity-40"><Download className="w-4 h-4" /> {exporting ? "EXPORTING…" : "EXPORT"}</button>
          {staffRole === "admin" ? (
            <button onClick={() => setImportOpen(true)} className="flex items-center gap-2 px-3 h-10 bg-white border-2 border-[#0A6B52] text-[#0A6B52] font-mono font-bold text-sm uppercase tracking-wider hover:bg-[#0A6B52] hover:text-white"><Upload className="w-4 h-4" /> DATABASE IMPORT</button>
          ) : (
            <button onClick={() => alert("CSV Import is admin-only.")} className="flex items-center gap-2 px-3 h-10 bg-white border-2 border-[#0A6B52] text-[#0A6B52] font-mono font-bold text-sm uppercase tracking-wider opacity-60" title="Admin only">DATABASE IMPORT</button>
          )}
          {staffRole === "admin" ? (
            <button onClick={() => setModalOpen(true)} className="flex items-center gap-2 px-4 h-10 bg-[#6C1C1F] text-white border-2 border-[#6C1C1F] font-mono font-bold text-sm uppercase tracking-wider hover:brightness-110"><Plus className="w-4 h-4" /> ADD RECORD</button>
          ) : (
            <button onClick={() => alert("Adding records requires Admin role.")} className="flex items-center gap-2 px-4 h-10 bg-[#6C1C1F] text-white border-2 border-[#6C1C1F] font-mono font-bold text-sm uppercase tracking-wider opacity-60" title="Admin role required"><Plus className="w-4 h-4" /> ADD RECORD</button>
          )}
        </div>

        {showFilters && (
          <div className="shrink-0 flex flex-wrap items-end gap-3 px-4 sm:px-6 py-4 bg-[#F0E8D0] border-b border-stone-300">
            ...
            {hasFilters && <button onClick={clearFilters} className="flex items-center gap-1.5 h-9 px-4 border-2 border-[#CC0000] text-[#CC0000] font-mono font-bold text-sm uppercase hover:bg-[#CC0000] hover:text-white"><X className="w-3.5 h-3.5" /> CLEAR</button>}
          </div>
        )}
            ...
            <thead className="bg-[#1A1A1A] text-[#F0E8D0] sticky top-0 z-10">
              <tr>
                {["DATE", "IMAGE", "MODIFIED", "TYPE", "CLIENT CODE", "CASE NO", "TM/CPR", "CLASS", "APPLICATION", "STATUS", "SUB-STATUS", "AGENT", "CITY", "TM FORM IPO (REGISTRY MATCHES)", "JOURNAL"].map((heading) => (
                  <th key={heading} className="px-3 py-2.5 border-r border-[#333] font-bold tracking-wider text-sm last:border-r-0 select-none">
                    {heading}
                  </th>
                ))}
              </tr>
                ...
                records.map((record, index) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-2 py-1 border-r border-[#0C0C0C]/10"><div className="w-9 h-9 border border-[#0C0C0C]/30 bg-[#F0E8D0] overflow-hidden flex items-center justify-center">{record.image ? <img src={record.image} alt="" className="w-full h-full object-contain" /> : <span className="text-sm text-[#9d9488]">—</span>}</div></td>
                      <td className="px-3 py-2 border-r border-[#0C0C0C]/10 text-[#6d6658] text-sm uppercase">{formatDateShort(record.updatedAt)}</td>
                      ...
                      <td className="px-3 py-2 border-r border-[#0C0C0C]/10"><span className={`inline-block px-1.5 py-0.5 text-sm font-bold border border-[#0C0C0C]/30 uppercase ${STAGE_BADGE[record.stage] ?? "bg-[#E8DFC7]"}`}>{record.stage}</span></td>
                          ...
                          {activeForms.length ? activeForms.map((form) => {
                            ...
                            return (
                              <div key={form} className="flex flex-col gap-0.5">
                                <span className="px-1.5 py-0.5 bg-[#B0740E]/15 border border-[#B0740E] text-[#6C1C1F] text-sm font-bold uppercase inline-block w-fit">{form}</span>
                                {formattedDate && (
        ...
        <div className="shrink-0 flex items-center justify-between px-4 sm:px-6 py-3 bg-[#E8DFC7] border-t-2 border-[#0C0C0C]">
          <span className="font-mono text-sm text-[#6d6658] font-bold uppercase tracking-widest">PAGE {page} OF {totalPages} · {total} RECORDS · SORTED BY DATE (NEWEST FIRST)</span>
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-2">
              <label className="font-mono text-sm text-[#6d6658] font-bold uppercase">SHOW:</label>
              <select 
                value={pageSize} 
                onChange={(e) => handlePageSizeChange(Number(e.target.value))}
                className="h-8 px-2 bg-white border border-stone-300 font-mono text-sm focus:outline-2 focus:outline-[#6C1C1F]"
              >
            ...
            <div className="flex gap-2">
              <button onClick={() => setPage(1)} disabled={page === 1 || isFetching} className="flex items-center gap-1 px-3 py-1.5 border border-stone-300 bg-white font-mono text-sm font-bold uppercase disabled:opacity-40">FIRST</button>
              <button onClick={() => setPage((value) => Math.max(1, value - 1))} disabled={page === 1 || isFetching} className="flex items-center gap-1 px-3 py-1.5 border border-stone-300 bg-white font-mono text-sm font-bold uppercase disabled:opacity-40"><ChevronLeft className="w-3.5 h-3.5" /> PREV</button>
              <button onClick={() => setPage((value) => Math.min(totalPages, value + 1))} disabled={page >= totalPages || isFetching} className="flex items-center gap-1 px-3 py-1.5 border border-stone-300 bg-white font-mono text-sm font-bold uppercase disabled:opacity-40">NEXT <ChevronRight className="w-3.5 h-3.5" /></button>
              <button onClick={() => setPage(totalPages)} disabled={page >= totalPages || isFetching} className="flex items-center gap-1 px-3 py-1.5 border border-stone-300 bg-white font-mono text-sm font-bold uppercase disabled:opacity-40">LAST</button>
            </div>
          </div>
        </div>
      </div>
