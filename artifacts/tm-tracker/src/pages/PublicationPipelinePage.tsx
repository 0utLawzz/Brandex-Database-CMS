import { formatTmNumber } from "@/lib/utils";
import {
  ...
  return (
    <AppShell>
          ...
          <div className="flex items-center gap-3 flex-wrap">
            {!isViewer ? (
              <>
                ...
                <button
                  ...
                  className="inline-flex items-center gap-2 border-2 border-[#6C1C1F] bg-[#FFF0D0] px-4 py-2 font-mono text-sm font-bold uppercase text-[#6C1C1F] hover:bg-[#6C1C1F] hover:text-white transition-colors"
                >
              ...
              <>
                ...
                <button
                  disabled
                  className="inline-flex items-center gap-2 border-2 border-[#6C1C1F]/40 bg-[#FFF0D0]/40 px-4 py-2 font-mono text-sm font-bold uppercase text-[#6C1C1F]/40 cursor-not-allowed"
                  title="Admin role required"
                >
                  <Upload className="h-4 w-4" /> JOURNAL IMPORT
                </button>
            ...
            <div className="flex items-center gap-4 ml-auto flex-wrap">
              <div className="flex items-center gap-2">
                ...
                <span className="font-mono text-sm text-[#6d6658]">PENDING ({pendingCount})</span>
              </div>
              <div className="flex items-center gap-2">
                <div className="w-3 h-3 border-2 border-[#6C1C1F] bg-[#FFF0D0]" />
                <span className="font-mono text-sm text-[#6d6658]">OVERDUE ({overdueCount})</span>
              </div>
              <div className="flex items-center gap-2">
                <div className="w-3 h-3 border border-stone-300 bg-[#E8DFC7]" />
                <span className="font-mono text-sm text-[#6d6658]">DONE ({doneCount})</span>
              </div>
            </div>
          </div>
        </div>
          ...
          {isLoading ? (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              {records.map((record) => (
                <div
                  key={record.id}
                  className="border border-stone-300 bg-white shadow-none hover:shadow-[3px_3px_0_#0C0C0C] transition-shadow cursor-pointer"
                  onClick={() => {
                ...
                >
                  ...
                  <div className="px-4 py-3 border-b border-stone-300 bg-[#E8DFC7]">
                    <div className="flex items-center justify-between gap-3">
                      <div className="flex-1">
                        <div className="text-sm uppercase tracking-widest text-[#6d6658] mb-0.5">JOURNAL NO</div>
                        <div className="font-serif text-lg font-bold text-[#0A6B52] leading-none">
                          {record.journalNumber || "—"}
                        </div>
                      </div>
                      <div className="flex-1 text-right">
                        <div className="text-sm uppercase tracking-widest text-[#6d6658] mb-0.5">PUB DATE</div>
                  ...
                  <div className="px-4 py-2 border-b border-[#0C0C0C]/20 flex items-center gap-3 font-mono text-sm">
                  ...
                  {/* Application identity and accessible enlargement */}
                  {record.image && <a href={record.image} target="_blank" rel="noreferrer" onClick={e=>e.stopPropagation()} className="block px-4 pt-4" aria-label={`Enlarge image for ${record.appName}`}><img src={record.image} alt={record.appName} className="h-28 w-40 object-contain border border-stone-200 rounded" /></a>}
                  {/* Application Name */}
                  <div className="px-4 py-2 border-b border-[#0C0C0C]/20">
                    <div className="font-bold text-xl text-[#0C0C0C] break-words">{record.appName}</div>
                  </div>

                  {/* Secondary information - TM Number and Class */}
                  <div className="px-4 py-2 border-b border-[#0C0C0C]/20 grid grid-cols-2 gap-2 font-mono text-sm">
                    <div>
                      <div className="text-sm uppercase tracking-widest text-[#6d6658]">TM NO</div>
                      <div className="font-bold text-[#0C0C0C]">{formatTmNumber(record.tmCprNo)}</div>
                    </div>
                    <div>
                      <div className="text-sm uppercase tracking-widest text-[#6d6658]">CLASS</div>
                      <Link onClick={e=>e.stopPropagation()} className="font-semibold underline text-[#6C1C1F]" href={`/database?appClass=${encodeURIComponent(record.appClass)}`}>{record.appClass || "—"} · View class cases</Link>
                    </div>
                  </div>
                  ...
                  <div className="px-4 py-2 border-b border-[#0C0C0C]/20 grid grid-cols-2 gap-2 font-mono text-sm">
                    <div>
                      <div className="text-sm uppercase tracking-widest text-[#6d6658]">APPLICANT</div>
                      <div className="text-[#0C0C0C] truncate">{record.clientName || "—"}</div>
                    </div>
                    <div>
                      <div className="text-sm uppercase tracking-widest text-[#6d6658]">AGENT</div>
                  ...
                  <div className="px-4 py-2 border-b border-[#0C0C0C]/20 flex items-center gap-2">
                    <span className="inline-block px-2 py-0.5 text-sm font-bold uppercase border border-[#0C0C0C]/20 bg-[#E8DFC7]">
                      {record.stage}
                    </span>
                    {record.subStage && (
                      <span className="text-sm font-bold text-[#6C1C1F] truncate">
                        {record.subStage}
                      </span>
                    )}
                  </div>
                  ...
                  <div className="px-4 py-2 border-b border-[#0C0C0C]/20 flex items-center justify-between font-mono text-sm">
                    <div>
                      <div className="text-sm uppercase tracking-widest text-[#6d6658]">DEADLINE</div>
                      <div className="font-bold text-[#0C0C0C]">{record.oppositionDeadline || "—"}</div>
                    </div>
                    <div className="text-right">
                      <div className="text-sm uppercase tracking-widest text-[#6d6658]">DAYS REM.</div>
                  ...
                  {/* Demand Note Status */}
                  <div className="px-4 py-2 border-b border-[#0C0C0C]/20 flex items-center gap-2 font-mono text-sm">
                  ...
                  <div className="px-4 py-2 flex items-center justify-between">
                    <span
                      className={`inline-block px-2 py-0.5 text-sm font-bold uppercase border ${STATUS_BADGE[record.status]}`}
                    >
                    ...
                    <div className="flex items-center gap-1.5">
                      ...
                      <Link
                        ...
                        className="inline-flex items-center gap-1 border border-stone-300 bg-white px-2 py-1 font-mono text-sm font-bold uppercase hover:bg-[#0C0C0C] hover:text-white text-[#6C1C1F]"
              ...
              <tbody>
                {records.map((record) => (
                  <tr key={record.id} className="border-b border-[#333]/30">
                    ...
                    <td className="px-2 py-1 border-r border-[#333]/30">{formatTmNumber(record.tmCprNo)}</td>
      ...
      {selectedRecord && (
        ...
        >
          <section
            className="w-full max-w-2xl border-3 border-[#0C0C0C] bg-[#F0E8D0] p-5 shadow-none max-h-[90vh] overflow-y-auto"
            onClick={(event) => event.stopPropagation()}
          >
            {/* Modal header */}
            <div className="flex items-start gap-3 border-b border-stone-300 pb-3 mb-4">
              <div className="min-w-0 flex-1">
                ...
                <div className="mt-1.5 font-mono text-sm font-bold uppercase text-[#6d6658]">
              ...
              <button
                type="button"
                onClick={() => setSelectedRecord(null)}
                className="border border-stone-300 bg-white p-1 hover:bg-[#0C0C0C] hover:text-white"
                aria-label="Close"
              >
                <X className="h-4 w-4" />
              </button>
            ...
            {/* Case info grid */}
            <div className="grid grid-cols-2 gap-2.5 border border-stone-300 bg-white p-3 font-mono text-sm uppercase mb-3">
              <div>
                <span className="block text-sm font-bold text-[#6d6658]">TM / CPR NO</span>
                <strong>{selectedRecord.tmCprNo || "\u2014"}</strong>
              </div>
              <div>
                <span className="block text-sm font-bold text-[#6d6658]">CLASS</span>
                <strong>{selectedRecord.appClass || "\u2014"}</strong>
              </div>
              <div>
                <span className="block text-sm font-bold text-[#6d6658]">CLIENT</span>
                <strong>{selectedRecord.clientName || "\u2014"}</strong>
              </div>
              <div>
                <span className="block text-sm font-bold text-[#6d6658]">AGENT</span>
                <strong>{selectedRecord.agent || <span className="italic text-[#9d9488] normal-case">Unassigned</span>}</strong>
              </div>
              <div>
                <span className="block text-sm font-bold text-[#6d6658]">STAGE</span>
                <strong>{selectedRecord.stage || "\u2014"}</strong>
              </div>
              <div>
                <span className="block text-sm font-bold text-[#6d6658]">SUB-STAGE</span>
                <strong>{selectedRecord.subStage || "\u2014"}</strong>
              </div>
            </div>

            {/* Journal / Publication info */}
            <div className="border-2 border-[#0A6B52] bg-[#D8F2E8]/30 p-3 font-mono text-sm uppercase mb-3">
              <div className="text-sm font-bold text-[#0A6B52] mb-2 flex items-center gap-1.5">
                <FileText className="h-3 w-3" /> JOURNAL / PUBLICATION INFO
              </div>
              <div className="grid grid-cols-2 gap-2">
                <div>
                  <span className="block text-sm font-bold text-[#6d6658]">JOURNAL NO</span>
                  <strong>{selectedRecord.journalNumber || "\u2014"}</strong>
                </div>
                <div>
                  <span className="block text-sm font-bold text-[#6d6658]">PUBLICATION DATE</span>
                  <strong>{selectedRecord.publicationDate || "\u2014"}</strong>
                </div>
                <div>
                  <span className="block text-sm font-bold text-[#6d6658]">OPPOSITION DEADLINE</span>
                  <strong>{selectedRecord.oppositionDeadline || "\u2014"}</strong>
                </div>
                <div>
                  <span className="block text-sm font-bold text-[#6d6658]">DAYS REMAINING</span>
            ...
            {/* Demand Note section */}
            <div className="border border-stone-300 bg-white p-3 mb-3">
              <h3 className="font-mono text-sm font-bold uppercase text-[#0C0C0C] mb-3">
                Demand Note Status
              </h3>
              {selectedRecord.demandNoteReceived ? (
                <div className="flex items-center justify-between flex-wrap gap-2">
                  ...
                  <button
                    ...
                    className="border-2 border-[#CC0000] bg-white px-3 py-1.5 font-mono text-sm font-bold uppercase text-[#CC0000] hover:bg-[#CC0000] hover:text-white transition-colors disabled:opacity-50"
                  >
                    CLEAR
                  </button>
                </div>
                ...
                <div className="space-y-3">
                  <div className="flex items-center gap-2 text-[#6d6658]">
                    ...
                    <span className="font-mono text-sm">Not yet received</span>
                  </div>
                  {!isViewer ? (
                    <div className="flex items-center gap-2 flex-wrap">
                      <label className="font-mono text-sm font-bold uppercase text-[#6d6658]">
                        Received Date:
                      </label>
                      <input
                        type="date"
                        ...
                        className="h-8 px-2 border border-stone-300 font-mono text-sm focus:outline-2 focus:outline-[#6C1C1F]"
                      />
                      <button
                        onClick={() =>
                        ...
                        className="border-2 border-[#0A6B52] bg-[#D8F2E8] px-3 py-1.5 font-mono text-sm font-bold uppercase text-[#0A6B52] hover:bg-[#0A6B52] hover:text-white transition-colors disabled:opacity-50"
                      >
                        MARK RECEIVED
                      </button>
                    </div>
            ...
            <div className="mt-4 pt-3 border-t-2 border-[#0C0C0C] flex justify-end">
              <Link
                ...
                className="inline-flex items-center gap-1.5 border border-stone-300 bg-[#6C1C1F] text-white px-4 py-2 font-mono text-sm font-bold uppercase hover:bg-[#501416] transition-colors"
              >
      ...
      {showMatchDialog && (
        ...
        >
          <section
            className="w-full max-w-md border-3 border-[#0C0C0C] bg-[#F0E8D0] p-5 shadow-none"
            onClick={(event) => event.stopPropagation()}
          >
            <div className="flex items-start gap-3 border-b border-stone-300 pb-3 mb-4">
              <div className="min-w-0 flex-1">
            ...
            <div className="flex flex-col gap-3">
              <button
                ...
                className="border-2 border-[#0A6B52] bg-[#D8F2E8] px-4 py-3 font-mono text-sm font-bold uppercase text-[#0A6B52] hover:bg-[#0A6B52] hover:text-white transition-colors disabled:opacity-50"
              >
                {matchLoading ? "RUNNING\u2026" : "RUN JOURNAL MATCH"}
              </button>
              <p className="font-mono text-sm text-[#6d6658]">
